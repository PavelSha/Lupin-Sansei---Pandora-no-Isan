...
.import sub_BBFE_check_room                ; bank 06 (Page 2)
...
C - - - - - 0x01C08E 07:C07E: 86 33     STX vLowMenuCounter          ; clear
...
C - - - - - 0x01C0A7 07:C097: 30 03     BMI bra_C09C_skip                  ; Branch If mode=cutscene
...
C - - - - - 0x01C121 07:C111: C9 0B     CMP #$0B                           ; CONSTANT - ???
...
C - - - - - 0x01C12A 07:C11A: 85 33     STA vLowMenuCounter   ; Initializes a counter.
C - - - - - 0x01C12C 07:C11C: A9 02     LDA #$02              ;
C - - - - - 0x01C12E 07:C11E: 85 34     STA vHighMenuCounter  ; Initializes a time of a demo scene.
bra_C120_repeat:
...
C - - - - - 0x01C137 07:C127: A5 34     LDA vHighMenuCounter
...
- - - - - - 0x01C148 07:C138: F0 0E     BEQ bra_C148_skip
- - - - - - 0x01C14A 07:C13A: 45 20     EOR v_last_p2_btn_pressed
- - - - - - 0x01C14C 07:C13C: 29 02     AND #$02
- - - - - - 0x01C14E 07:C13E: F0 08     BEQ bra_C148_skip
...
C - - - - - 0x01C675 07:C665: 10 F7     BPL bra_C65E_loop                       ; If Register X < 0xF0
...
C - - - - - 0x01C892 07:C882: 10 DF     BPL bra_C863_loop                     ; If $000F < 0xF0
C - - - - - 0x01C894 07:C884: 60        RTS                                   ;

sub_C885:
C - - - - - 0x01C895 07:C885: A5 37     LDA vGameMode         ;
C - - - - - 0x01C897 07:C887: 10 20     BPL bra_C8A9          ; Branch If in game
C - - - - - 0x01C899 07:C889: C6 33     DEC vLowMenuCounter   ;
C - - - - - 0x01C89B 07:C88B: D0 06     BNE @bra_C893_skip    ; If vLowMenuCounter != 0x00
C - - - - - 0x01C89D 07:C88D: A5 34     LDA vHighMenuCounter  ;
C - - - - - 0x01C89F 07:C88F: F0 02     BEQ @bra_C893_skip    ; If vHighMenuCounter == 0x00
C - - - - - 0x01C8A1 07:C891: C6 34     DEC vHighMenuCounter  ;
@bra_C893_skip:
...
C - - - - - 0x01CCAD 07:CC9D: 85 2E     STA vCorridorCounter
...
; out: Register A - vEnemyRNG
sub_D064_generate_rng:
C - - - - - 0x01D074 07:D064: A5 35     LDA vEnemyRNG ;
C - - - - - 0x01D076 07:D066: 85 12     STA ram_0012  ; cache
C - - - - - 0x01D078 07:D068: 0A        ASL           ;
C - - - - - 0x01D079 07:D069: 0A        ASL           ; * 4
C - - - - - 0x01D07A 07:D06A: 18        CLC           ;
C - - - - - 0x01D07B 07:D06B: 65 12     ADC ram_0012  ;
C - - - - - 0x01D07D 07:D06D: 18        CLC           ;
C - - - - - 0x01D07E 07:D06E: 69 13     ADC #$13      ;
C - - - - - 0x01D080 07:D070: 85 35     STA vEnemyRNG ;
...
C - - - - - 0x01D1AD 07:D19D: C6 27     DEC vLowViewPortPosX
...
C - - - - - 0x01D2F2 07:D2E2: 10 F8     BPL @bra_D2DC_loop                              ; If Register Y < 0xF0
...
C - - - - - 0x01D2F6 07:D2E6: 48        PHA          ; store x (D2E6)
...
C - - - - - 0x01D309 07:D2F9: B0 05     BCS bra_D300_skip ; If vScreenChrPosY >= 0x30
...
; In: 0x0000 - vScreenChrPosY
; In: 0x0001 - vLowChrPosX
; In: 0x0004 - {0x00, 0x0C, 0x18, 0x24, 0x30, 0x3C, 0x48, 0x54, 0x60, 0x6C, 0x78, 0x84, 0x90, 0x9C, 0xA8, 0xB4}
...
; in 0x0000 - (ChrPosY + an increment)
sub_D36A:
loc_D36A:
...
C - - - - - 0x01D385 07:D375: 48        PHA               ; store A
C - - - - - 0x01D386 07:D376: C9 01     CMP #$01
C - - - - - 0x01D388 07:D378: D0 02     BNE bra_D37C_skip ; If Register A != 0x01
C - - - - - 0x01D38A 07:D37A: 68        PLA               ; retrieve A
C - - - - - 0x01D38B 07:D37B: 60        RTS

bra_D37C_skip:
C - - - - - 0x01D38C 07:D37C: A9 F8     LDA #$F8
C - - - - - 0x01D38E 07:D37E: 20 9F D3  JSR sub_D39F_increment_by_posX
...
sub_D3AD:
C D 2 - - - 0x01D3BD 07:D3AD: 20 9F D3  JSR sub_D39F_increment_by_posX
...
C - - - - - 0x01D40C 07:D3FC: 98        TYA                              ; v_cache_reg_y to A
...
C - - - - - 0x01DB1F 07:DB0F: D0 03     BNE bra_DB14_skip
...
- D 2 - I - 0x01DB44 07:DB34: 43 DB     .addr loc_DB44 - 1
...
C - - - - - 0x01DB56 07:DB46: 20 AB E5  JSR sub_E5AB_add_short_chr_y_positions
C - - - - - 0x01DB59 07:DB49: D0 07     BNE bra_DB52_skip
loc_DB4B:
...
C - - - - - 0x01DBED 07:DBDD: F0 07     BEQ @bra_DBE6_skip
...
C - - - - - 0x01DBF5 07:DBE5: E8        INX                ; increment the offset of the sprite address
@bra_DBE6_skip:
...
C - - - - - 0x01DEC9 07:DEB9: 85 2E     STA vCorridorCounter
...
C - - - - - 0x01E043 07:E033: E6 2E     INC vCorridorCounter
bra_E035_RTS:
...
bra_E33E:
C - - - - - 0x01E34E 07:E33E: 20 64 D0  JSR sub_D064_generate_rng
...
C - - - - - 0x01E3F0 07:E3E0: 86 2E     STX vCorridorCounter
bra_E3E2:
...
sub_E5A5:
C - - - - - 0x01E5B5 07:E5A5: 20 AB E5  JSR sub_E5AB_add_short_chr_y_positions
...
C - - - - - 0x01E71B 07:E70B: A5 2E     LDA vCorridorCounter
...
loc_EBC3:
C D 3 - - - 0x01EBD3 07:EBC3: A5 2E     LDA vCorridorCounter
...
C - - - - - 0x01EC0B 07:EBFB: A5 2E     LDA vCorridorCounter
...
C - - - - - 0x01EE4F 07:EE3F: F0 CE     BEQ bra_EE0F                          ; If Register A == 0x93
...
C - - - - - 0x01EF7E 07:EF6E: 10 0A     BPL bra_EF7A_RTS     ; If v_item_on_screen >= 0x00 && v_item_on_screen < 0xF0
...
C - - - - - 0x01F05E 07:F04E: 10 FA     BPL @clear_item_loop                ; If Register X < 0xF0 (a loop condition)
...
bra_F18B:
C - - - - - 0x01F19B 07:F18B: 20 FE BB  JSR sub_BBFE_check_room ;
...
C - - - - - 0x01F27A 07:F26A: 24 35     BIT vEnemyRNG
...
tbl_F888:
- - - - - - 0x01F898 07:F888: 87 F8     .addr enemy_RTS ; Nobody  (0x00)
...
- - - - - - 0x01F8EA 07:F8DA: 87 F8     .addr enemy_RTS ; ???
- D 3 - - - 0x01F8EC 07:F8DC: 7D F7     .word $F77D ; Boss
- - - - - - 0x01F8EE 07:F8DE: 87 F8     .addr enemy_RTS ; ???
- D 3 - - - 0x01F8F0 07:F8E0: 79 F7     .word $F779 ; Boss
- - - - - - 0x01F8F2 07:F8E2: 87 F8     .addr enemy_RTS ; ???
- D 3 - - - 0x01F8F4 07:F8E4: 75 F7     .word $F775 ; Boss
- - - - - - 0x01F8F6 07:F8E6: 87 F8     .addr enemy_RTS ; ???
...
C - - - - - 0x01FC3D 07:FC2D: 20 FE BB  JSR sub_BBFE_check_room
...
tbl_FCBA:
- D 3 - - - 0x01FCCA 07:FCBA: 87 F8     .addr enemy_RTS ; Nobody  (0x00)
...
- D 3 - - - 0x01FD1C 07:FD0C: 87 F8     .addr enemy_RTS ; ??? (0x29)
- D 3 - - - 0x01FD1E 07:FD0E: DB EE     .word $EEDB ; Boss (level 2) (0x2A) Type A
- D 3 - - - 0x01FD20 07:FD10: 87 F8     .addr enemy_RTS ; ??? (0x2B)
- D 3 - - - 0x01FD22 07:FD12: E4 EE     .word $EEE4 ; Boss (level 3) (0x2C) Type A
- D 3 - - - 0x01FD24 07:FD14: 87 F8     .addr enemy_RTS ; ??? (0x2D)
- D 3 - - - 0x01FD26 07:FD16: ED EE     .word $EEED ; Boss (level 4) (0x2E) Type A
- D 3 - - - 0x01FD28 07:FD18: 87 F8     .addr enemy_RTS ; ??? (0x2F)
