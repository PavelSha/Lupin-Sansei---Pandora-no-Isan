...
Смотри RAM address - '$0046'.

# Позиционирование

В игре используется только *горизонтальный скроллинг*.
Учитывая, что размеры комнат могут быть больше одного игрового экрана, перемещение внутри комнат может происходить только по горизонтали.
Приставка Family Computer имеет разрешения 256x224 пикселей (NTSC) и 256x240 pixels (PAL).
При таких разрешениях положение пикселя по каждой из осей можно задать 1 байтом, причем по оси X это положение будет задаваться взаимо однозначно.
Учитывая всё вышеперечисленное, один из очевидных и простых способов позиционирования "перемещаемого" объекта в игре с горизонтальным скроллингом является тройка байт: положение объекта с точностью до экрана по оси X; номер экрана, где находится объект по оси X и положение объекта с точностью до экрана по оси Y.
Именно такой кодирования положение "перемещаемого" объекта и используется в игре.
В результате, при подсчетах положения по оси X мы работает с 16-битной арифметикой, по оси Y - с 8-битной.

Смотри RAM addresses - '$0066','$0068','$006A'.
